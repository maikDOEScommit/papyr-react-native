...
export async function shouldShowPopup(): Promise<boolean> {
...
export async function getPopupForDay(): Promise<PopupData | null> {
  const state = await getAppState();
  const totalDays = state.totalCommitments;
...
export async function markPopupAsShown(): Promise<void> {
  const state = await getAppState();
